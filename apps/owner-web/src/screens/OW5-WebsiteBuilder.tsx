import React, { useState, useEffect, useRef, Component, ReactNode } from 'react'; import { Card } from '@stayflo/ui';
...
const ROOM_COLOR   = '#1aab8b';
const COMMON_COLOR = '#EF9F27';
const DARK_COLOR   = '#374151';
const GARDEN_COLOR = '#3d7a2e';
const GREY_COLOR   = '#8b949e';
const DOOR_COLOR   = '#ef4444';
const WINDOW_COLOR = '#f59e0b';
                ...
                >
                  ...
                  >
                    {activeRooms.map(room => {
                      ...
                      // Auto-infer window facing from adjacent blocks
                      ...
                      if (isRoom && room.windows && room.windows.length > 0) {
                        ...
                        else if (neighbor?.type === 'Garden' || neighbor?.type === 'Parking') inferredFacing = 'Ground side';
                      }
                        ...
                        >
                          ...
                          {isRoom ? (
                            <div className="absolute inset-0 flex flex-col p-2 pointer-events-none select-none">
                              ...
                              {/* Bed cards — scaled to fit the room block */}
                              {room.beds > 0 && (() => {
                                const bedsCount = room.beds;
                                ...
                                return show ? (
                                  <div className="flex-1 flex items-center justify-center" style={{ gap: `${gapPx}px` }}>
                                    {Array(bedsCount).fill(0).map((_, bIdx) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          {isOccupied ? (
                                            <svg style={{ width: Math.max(8, bedW * 0.45), height: Math.max(8, bedW * 0.45) }} viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2">
                                              <circle cx="12" cy="7" r="4"/>
                                              <path strokeLinecap="round" d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/>
                                            </svg>
                                          ) : (
                                            <svg style={{ width: Math.max(8, bedW * 0.4), height: Math.max(8, bedW * 0.4) }} viewBox="0 0 24 24" fill="none" stroke="#2dd4bf" strokeWidth="2.5">
                            ...
                            <>
                              ...
                              {isRoad && (
                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none p-1 overflow-hidden">
                                  {room.w >= room.h ? <div className="w-full border-t-[3px] border-dashed border-white/70" /> : <div className="h-full border-l-[3px] border-dashed border-white/70" />}
                                  <span className="absolute text-[8px] font-black text-white/80 uppercase tracking-widest top-1 left-1">ROAD</span>
                                </div>
                              )}
                              {isBuilding && (
                                <div className="absolute inset-0 pointer-events-none opacity-30 overflow-hidden rounded" style={{ backgroundImage: 'repeating-linear-gradient(45deg,#fff 0,#fff 2px,transparent 0,transparent 8px)', backgroundSize: '10px 10px' }} />
                              )}
                              {isGarden && (
                              ...
                              {isParking && <span className="absolute inset-0 flex items-center justify-center text-white/50 text-3xl font-black pointer-events-none select-none">P</span>}
                              {isStaircase && (
                          ...
                          {/* ── Window markers: short amber bar with inner slot ── */}
