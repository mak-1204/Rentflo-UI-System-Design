...
export function OwnerWebTenants() {
  const [search, setSearch] = useState('');
  const [notif, setNotif] = useState<string | null>(null);

  // Load / initialize tenants list
  const [tenants, setTenants] = useState<Tenant[]>(() => {
    const saved = localStorage.getItem('rentflo_tenants_list');
    if (saved) {
  ...
  return (
    <div className="p-8 space-y-6 max-w-7xl mx-auto relative overflow-hidden text-left min-h-screen">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button style={{ background: '#1D9E75', color: '#FFFFFF' }} className="hover:bg-[#0F6E56]" onClick={handleAddTenant}>
