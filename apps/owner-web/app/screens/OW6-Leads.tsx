...
export function OwnerWebLeads() {
  const [notif, setNotif] = useState<string | null>(null);

  // Leads list in state (loaded from localStorage if exists)
  const [leads, setLeads] = useState<Lead[]>(() => {
    const saved = localStorage.getItem('rentflo_leads_list');
    if (saved) {
  ...
  const portfolioLink = `${window.location.origin}/portfolio/sunrise-pg/explore`;
  ...
  return (
    <div className="p-8 space-y-6 max-w-7xl mx-auto relative text-left">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button variant="outline" className="flex items-center gap-2" onClick={() => triggerToast('CSV Export triggered!')}>
