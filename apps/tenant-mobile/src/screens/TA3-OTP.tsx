import { useState } from 'react'; import { useNavigate } from 'react-router'; import { ChevronLeft } from 'lucide-react'; import { Button } from '@rentflo/ui';
import { InputOTP, InputOTPGroup, InputOTPSlot } from '@rentflo/ui';

export function TenantOTP() {
  const navigate = useNavigate();
  const [otp, setOtp] = useState('');
  
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col justify-center items-center p-6 text-left">
      <div className="w-full max-w-md bg-white p-8 rounded-xl border shadow-sm relative">
        <button onClick={() => navigate(-1)} className="mb-4 p-1 hover:bg-slate-105 rounded absolute top-4 left-4">
          <ChevronLeft className="w-6 h-6" style={{ color: '#111827' }} />
        </button>
        
        <div className="mb-8 mt-6 text-center">
          <h1 className="text-2xl font-bold mb-1.5" style={{ color: '#111827' }}>
            Enter OTP Code
          </h1>
          <p className="text-sm" style={{ color: '#6B7280' }}>
            Sent to +91 98765 43210
          </p>
        </div>
        
        <div className="space-y-6 flex flex-col items-center">
          <div className="flex justify-center w-full">
            <InputOTP maxLength={6} value={otp} onChange={setOtp}>
              <InputOTPGroup className="gap-2 justify-center w-full">
                <InputOTPSlot index={0} className="w-11 h-12 text-lg border-2" />
                <InputOTPSlot index={1} className="w-11 h-12 text-lg border-2" />
                <InputOTPSlot index={2} className="w-11 h-12 text-lg border-2" />
                <InputOTPSlot index={3} className="w-11 h-12 text-lg border-2" />
                <InputOTPSlot index={4} className="w-11 h-12 text-lg border-2" />
                <InputOTPSlot index={5} className="w-11 h-12 text-lg border-2" />
              </InputOTPGroup>
            </InputOTP>
          </div>
          
          <Button 
            className="w-full h-11 text-sm font-semibold uppercase tracking-wider"
            style={{ background: '#1D9E75', color: '#FFFFFF' }}
            onClick={() => navigate('/tenant-mobile/join-pg')}
            disabled={otp.length < 6}
          >
            Verify & Continue
          </Button>
        </div>
      </div>
    </div>
  );
}
